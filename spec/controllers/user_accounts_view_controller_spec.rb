describe UserAccountsViewController do
  ...
  before do
    ...
    stub_request(:get, "http://mon.tinymon.org/accounts/5/user_accounts.json").to_return(json: {
      :user_accounts => [
  ...
  it "should refresh on pull down" do
    wait 0.5 do
      reset_stubs
      stub_request(:get, "http://mon.tinymon.org/accounts/5/user_accounts.json").to_return(json: { :user_accounts => [{ :id => 10, :user => { :id => 10, :full_name => 'John Doe', :email => 'johndoe@example.com' } }] })
  ...
  it "should disclose user" do
    stub_request(:get, "http://mon.tinymon.org/users/10.json").to_return(json: { :id => 10, :full_name => 'John Doe' })
